...
def predict(model, obs, obs_prev, obs_prev2, action_prev, action_prev2, i, SAVE):
    model = model.eval()
    # Get image from observation
    image, front, overhead, left, right, wrist = get_image_from_observation(obs, obs_prev, obs_prev2, i)
    ...
    with torch.no_grad():
        pred = model((image, action_prev, action_prev2))
...
def get_image_from_observation(obs, obs_prev, obs_prev2, i):
	...
	if ((i % int(NUM_ITERS) == 0) or (i == 0)):
		...
		im11 = Image.fromarray(obs_prev2.front_rgb)
		im12 = Image.fromarray(obs_prev2.overhead_rgb)
		im13 = Image.fromarray(obs_prev2.left_shoulder_rgb)
		im14 = Image.fromarray(obs_prev2.right_shoulder_rgb)
		im15 = Image.fromarray(obs_prev2.wrist_rgb)
		
		
	im_actual = np.concatenate((im1, im2, im3, im4, im5), axis=2)
	im_previa_1 = np.concatenate((im6, im7, im8, im9, im10), axis=2)
	im_previa_2 = np.concatenate((im11, im12, im13, im14, im15), axis=2)
	image = np.concatenate((im_actual, im_previa_1, im_previa_2), axis=1)	
...
# Condicion de inicio
print('Reset Episode')	
print('Acciones desde t hasta t-2:')
...
while (not terminate) or (int(NUM_PRUEBAS) > int(its)) :
	...
	if ((int(NUM_ITERS) - int(num_iters_seguidas) == 0) or (terminate == True)):
		...
		obs_prev = obs
		obs_prev2 = obs
	action = predict(model, obs, obs_prev, obs_prev2, action_prev, action_prev2, i, SAVE)
