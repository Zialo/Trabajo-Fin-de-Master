...
FLAGS = flags.FLAGS
EPISODES = sys.argv[1] if len(sys.argv) > 1 else 50
VARIATIONS = sys.argv[2] if len(sys.argv) > 2 else 1
TASK = sys.argv[3] if len(sys.argv) > 3 else 'reach_target'
print("Preparing the " + str(EPISODES) + " episodes")
print("Preparing the " + str(VARIATIONS) + " variations")
print("Task to do: " + TASK)

flags.DEFINE_string('save_path',
                    '/home/hodei/RLBench/tools/rlbench_data/',
                    'Where to save the demos.')
                                      
flags.DEFINE_list('tasks', TASK,
                  'The tasks to collect. If empty, all tasks are collected.')
flags.DEFINE_list('image_size', [128, 128],
                  'The size of the images tp save.')
flags.DEFINE_enum('renderer',  'opengl3', ['opengl3', 'opengl'],
                  'The renderer to use. opengl does not include shadows, '
                  'but is faster.')
flags.DEFINE_integer('processes', 1,
                     'The number of parallel processes during collection.')
flags.DEFINE_integer('episodes_per_task', str(EPISODES),
                     'The number of episodes to collect per task.')
flags.DEFINE_integer('variations', str(VARIATIONS),
