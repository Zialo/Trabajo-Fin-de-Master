...
class MultiImage(nn.Module):
    ...
    def forward(self, x):
        ...
        f = torch.cat((f, x7), dim=1)
        return self.clf(f)

def predict(model, obs, obs_prev, action_prev, i):
    model = model.eval()
    # Get image from observation
    im1, im2, im3, im4, im5, im6 = get_image_from_observation(obs, obs_prev, i)
    ...
    with torch.no_grad():
        pred = model((im1, im2, im3, im4, im5, im6, action_prev))
...
def get_image_from_observation(obs, obs_prev, i):
	...
	if ((i % episode_length == 0) or (i == 0)):
		...
		im4 = Image.fromarray(obs_prev.front_rgb)
		im5 = Image.fromarray(obs_prev.left_shoulder_rgb)
		im6 = Image.fromarray(obs_prev.right_shoulder_rgb)
...
# Load model
model = torch.load('model_pytorch_vel_resnet34_norm', map_location ='cpu')
