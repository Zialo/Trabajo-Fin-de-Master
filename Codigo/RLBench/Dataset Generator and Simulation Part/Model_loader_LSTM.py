...
class MultiImage(nn.Module):
    def __init__(self, fe, clf, hidden, input_size, hidden_size):
    ...
    def forward(self, x):
        ...
        aux = [x2,x3,x4,x5,x6] # Cada uno de [1, 7]

        inputs = torch.zeros(5, np.shape(x2)[0], 7) # [5, 1, 7]

        for ii in range(np.shape(inputs)[0]):
            inputs[ii] = aux[ii]

        if np.shape(x2)[0] == 32:
            xlstm, hid = self.lstm(inputs, self.hidden)[-1][1]
        else:
            hidden = (torch.randn(2, np.shape(x2)[0], 7), torch.randn(2, np.shape(x2)[0], 7))
            xlstm, hid = self.lstm(inputs, hidden)[-1][1]
        #xlstm = torch.squeeze(xlstm)
        f1 = self.fe(x1)
        f = self.flatten(self.avg_pool(f1))
        f = torch.cat((f, xlstm), dim=1)
        return self.clf(f)


def predict(model, obs, obs_prev, action_prev, action_prev2, action_prev3, action_prev4, action_prev5, i, SAVE):
    hidden = (torch.randn(2, 1, 7), torch.randn(2, 1, 7))
    input_size = 7
    hidden_size = np.shape(hidden[0])[2]
    model = model.eval()
...
def get_image_from_observation(obs, obs_prev, i):
    ...
    if ((i % int(NUM_ITERS) == 0) or (i == 0)):
        im6 = Image.fromarray(obs.front_rgb)
        im7 = Image.fromarray(obs.overhead_rgb)
        im8 = Image.fromarray(obs.left_shoulder_rgb)
        im9 = Image.fromarray(obs.right_shoulder_rgb)
        im10 = Image.fromarray(obs.wrist_rgb)
    ...
    image = np.concatenate((im_actual, im_previa_1), axis=2)
...
# Las ACTION_PREV son para las acciones
...
while (not terminate) or (int(NUM_PRUEBAS) > int(its)) :
    ...
    if ((int(NUM_ITERS) - int(num_iters_seguidas) == 0) or (terminate == True)):
        ...
        print('Reset Episode')
        print('Acciones desde t hasta t-5: INICIO')
        action_prev = np.array([0,0,0,0,0,0,0], dtype="float32")
        action_prev2 = np.array([0,0,0,0,0,0,0], dtype="float32")
        action_prev3 = np.array([0,0,0,0,0,0,0], dtype="float32")
        action_prev4 = np.array([0,0,0,0,0,0,0], dtype="float32")
    ...
    print('Acciones desde t hasta t-5: TRAS CAMBIAR')
    print(action_prev)
    print(action_prev2)
    print(action_prev3)
    print(action_prev4)
