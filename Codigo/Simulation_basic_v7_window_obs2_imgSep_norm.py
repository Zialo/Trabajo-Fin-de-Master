...
scaler_obs = joblib.load("scaler_obs")
scaler_act = joblib.load("scaler_act")
...
aux_init_obs = np.array([0.5, 0.5, 0.5, 0.5], dtype="float32")
aux_init_act = np.array([0.5, 0.5, 0.5], dtype="float32")
...
while (not done) or (int(NUM_PRUEBAS) > int(its)):
    ...
    # Normalizamos las observaciones y acciones antes de pasarselas al modelo
    # OBSERVATIONS AND ACTIONS NORMALIZATION
    obs_act_n = (scaler_obs.transform(obs_act.reshape(1, -1)))[0]
    obs_prev_n = (scaler_obs.transform(obs_prev.reshape(1, -1)))[0]
    obs_prev2_n = (scaler_obs.transform(obs_prev2.reshape(1, -1)))[0]
    action_prev_n = (scaler_act.transform(action_prev.reshape(1, -1)))[0]
    
    # Control de inicio de las observaciones
    
    if np.array_equal(obs_prev_n, aux_init_obs): obs_prev_n = np.array([0, 0, 0, 0], dtype="float32")
    if np.array_equal(obs_prev2_n, aux_init_obs): obs_prev2_n = np.array([0, 0, 0, 0], dtype="float32")
    if np.array_equal(action_prev_n, aux_init_act): action_prev_n = np.array([0, 0, 0], dtype="float32")
  
    action = predict(model, env, env_prev, action_prev_n, obs_act_n, obs_prev_n, obs_prev2_n, i, SAVE)
    action_norm = np.array(action, dtype="float32")
    '''
    # Desnormalizamos la accion predicha para poder pasarsela al simulador bien
    # ACTION DESNORMALIZATION
    action_norm = (scaler_act.inverse_transform(action.reshape(1, -1)))[0]
    '''
    print('###############################################################################')
    print('NORMALIZACION DE LOS DATOS')
    print('OBS ACT: ' , obs_act , ' -> ' , obs_act_n)
    print('OBS PREV: ' , obs_prev , ' -> ' , obs_prev_n)
    print('OBS PREV_2: ' , obs_prev2 , ' -> ' , obs_prev2_n)
    print('ACTION PREV: ' , action_prev , ' -> ' , action_prev_n)
    print('DESNORMALIZACION DE LA ACCION PREDICHA')
    print('ACTION PREDICHA: ' , action , ' -> ' , action_norm)
    print('###############################################################################')

    env_prev = env
    
    action_norm_aux = list(action_norm)
    action_norm_aux.append(0.1)
    action_norm = np.array(action_norm_aux, dtype='float32')
