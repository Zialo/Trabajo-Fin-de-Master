...
def predict(model, env, env_prev, action_prev, i, SAVE):
    ...
    transformations = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.50167945, 0.40269135, 0.37946385),
                             (0.17415733, 0.23276974,  0.2364808))
    ])
...
while (not done) or (int(NUM_PRUEBAS) > int(its)):
    ...
    if near_object == 1.0 or near_object2 >= 0.95:
        nearness +=1
    ...
    print('Done: ', str(near_object), str(near_object2))
